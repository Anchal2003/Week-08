...
function rollDieSimulation() {
    ...
    while (!maxCountReached) {
        ...
        result[roll]++; // Increment the count for the rolled number

        // Check if any number has reached the maximum count
        if (result[roll] === maxRolls) {
    ...
    for (const [number, count] of Object.entries(result)) {
        if (count > maxCount) {
            maxCount = count;
            maxNumber = number;
        }
    ...
    // Print the result
    console.log("Die Roll Results:", result);
